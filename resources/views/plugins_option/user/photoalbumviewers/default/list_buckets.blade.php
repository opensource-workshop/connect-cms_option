...
@if ($plugin_buckets->isEmpty())
    <div class="alert alert-warning">
        ...
        選択画面から、使用するフォトアルバムビューアを選択するか、作成してください。
    </div>
@else
    <form action="{{url('/')}}/redirect/plugin/photoalbumviewers/changeBuckets/{{$page->id}}/{{$frame_id}}#frame-{{$frame->id}}" method="POST" class="">
        {{ csrf_field() }}
        <input type="hidden" name="redirect_path" value="{{url('/')}}/plugin/photoalbumviewers/listBuckets/{{$page->id}}/{{$frame_id}}#frame-{{$frame_id}}">
        ...
        <div class="text-center">
            ...
            <button type="submit" class="btn btn-primary"><i class="fas fa-check"></i> 表示フォトアルバムビューア変更</button>
        </div>
    </form>
@endif
@endsection
